...
const urnFk = { mysqlColumn: 'urn_no', mongoField: 'urnNo' } as const;
...
export const TABLE_REGISTRY: TableDefinition[] = [
  ...
  processTable(
    ...
    'process_ps_stakeholder_edu_awerness_id',
    'processPsStakeholderEduAwarnessId',
  ),
  ...
  rawMaterialsTable(
    ...
    'raw_materials_utilization_id',
    'rawMaterialsUtilizationId',
  ),
  rawMaterialsTable(
...
export function getTablesByPhase(phase?: number): TableDefinition[] {
  const sorted = [...TABLE_REGISTRY].sort((a, b) => a.phase - b.phase);
  if (phase === undefined) return sorted;
  return sorted.filter((t) => t.phase === phase);
}

export function getTableDefinition(mysqlTable: string): TableDefinition | undefined {
  return TABLE_REGISTRY.find((t) => t.mysqlTable === mysqlTable);
}
