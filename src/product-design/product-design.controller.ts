...
export class ProductDesignController {
  ...
  @Post()
  @UseInterceptors(
    AnyFilesInterceptor(productPerformanceMultipartMemoryMulterOptions(40)),
  )
  @ApiOperation({
    summary: 'Create or update product design data',
    description:
      'Saves product design for a URN. **measuresAndBenefits** replaces all measure rows for the URN (send the full list). ' +
      'Use **ecoVisionFile** and **supportingDesignFile** (repeat per file). ' +
      'Send **existingEcoVisionDocumentIds** / **existingSupportingDocumentIds** (JSON arrays of productDocumentId) to keep docs on text-only saves; omit to keep all, `[]` to clear unlisted. ' +
      'Legacy **files** + **ecoVisionFilesCount**: eco parts first, then supporting.',
  })
  ...
  @ApiBody({
    schema: {
      ...
      properties: {
        ...
        strategies: {
          type: 'string',
          description: 'Strategies text (preferred)',
        ...
        ecoVisionFile: {
          ...
          description:
            'Eco vision file(s). Repeat per file → eco_vision_upload (max 20).',
        },
        supportingDesignFile: {
          type: 'array',
          items: { type: 'string', format: 'binary' },
          description:
            'Supporting design file(s). Repeat per file → supporting_documents (max 20).',
        },
        ecoVisionFilesCount: {
          type: 'number',
