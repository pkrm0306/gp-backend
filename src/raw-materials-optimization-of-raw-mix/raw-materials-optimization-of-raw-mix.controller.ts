...
export class RawMaterialsOptimizationOfRawMixController {
  constructor(private readonly service: RawMaterialsOptimizationOfRawMixService) {}

  @Post()
  @ApiOperation({
    summary: 'Create raw materials optimization of raw mix record (per URN)',
  })
  @ApiBody({ type: CreateRawMaterialsOptimizationOfRawMixDto })
  @ApiResponse({ status: 201, description: 'Created successfully' })
  async create(
    @CurrentUser() user: any,
    @Body() dto: CreateRawMaterialsOptimizationOfRawMixDto,
  ) {
    if (!user?.vendorId) {
