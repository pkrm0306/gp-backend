...
export class RawMaterialsRegionalMaterialsController {
  ...
  @Post()
  @ApiOperation({
    summary: 'Create raw materials regional materials record (per URN)',
  })
  @ApiBody({ type: CreateRawMaterialsRegionalMaterialsDto })
  @ApiResponse({ status: 201, description: 'Created successfully' })
  async create(
    @CurrentUser() user: any,
    @Body() dto: CreateRawMaterialsRegionalMaterialsDto,
  ) {
    if (!user?.vendorId) {
  ...
  @Get(':urn_no')
  @ApiOperation({
    summary: 'List raw materials regional materials records by URN',
  })
