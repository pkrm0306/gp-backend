...
export class StatesService {
  ...
  async searchByName(stateName: string, countryId?: string): Promise<StateDocument[]> {
    try {
      ...
      if (sampleState) {
        ...
        console.log('[State Search] state_name value:', (sampleObj as any).state_name);
      } else {
      ...
      if (countryId) {
        try {
          ...
          if (country) {
            ...
            countryConditions.push({ countryId: new Types.ObjectId(countryId) });
      ...
      let query: any = { stateName: new RegExp(`^${this.escapeRegex(searchTerm)}$`, 'i') };
      if (countryFilter) {
        query = { $and: [query, countryFilter] };
      }
      
      let results = await this.stateModel.find(query).sort({ stateName: 1 }).exec();
      ...
      if (results.length === 0) {
        ...
        query = { state_name: new RegExp(`^${this.escapeRegex(searchTerm)}$`, 'i') };
        if (countryFilter) {
          query = { $and: [query, countryFilter] };
        }
        results = await this.stateModel.find(query).sort({ state_name: 1 }).exec();
      }
      ...
      for (const fieldQuery of searchFields) {
        ...
        results = await this.stateModel.find(query).sort({ stateName: 1 }).exec();
      ...
      if (words.length > 0) {
        ...
        for (const fieldName of fieldNames) {
          const orConditions = wordRegexes.map(regex => ({ [fieldName]: regex }));
      ...
      const fieldNames = ['stateName', 'name', 'state_name'];
      for (const fieldName of fieldNames) {
        const variationConditions = variationRegexes.map(regex => ({ [fieldName]: regex }));
      ...
      let allStates = await this.stateModel.find(countryFilter || {}).sort({ stateName: 1 }).exec();
      const similarStates = allStates
        .map(state => {
          // Try multiple field names for similarity
          const stateObj = state.toObject();
          const stateNameValue = stateObj.stateName || (stateObj as any).name || (stateObj as any).state_name || '';
          const stateNameLower = stateNameValue.toLowerCase();
          return {
            state,
            similarity: this.calculateSimilarity(searchTerm.toLowerCase(), stateNameLower),
          };
        })
