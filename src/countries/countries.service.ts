  ...
  async searchByName(countryName: string): Promise<CountryDocument[]> {
    try {
      ...
      if (sampleCountry) {
        ...
        console.log(
          '[Country Search] Sample country document:',
          JSON.stringify(sampleObj, null, 2),
        );
        console.log(
          '[Country Search] Available fields:',
          Object.keys(sampleObj),
        );
        console.log(
          '[Country Search] countryName value:',
          sampleObj.countryName,
        );
        console.log('[Country Search] name value:', (sampleObj as any).name);
        console.log(
          '[Country Search] country_name value:',
          (sampleObj as any).country_name,
        );
      } else {
        console.log(
          '[Country Search] WARNING: No countries found in database at all!',
        );
        return [];
      }
      ...
      let results = await this.countryModel
        .find({
          countryName: new RegExp(`^${this.escapeRegex(searchTerm)}$`, 'i'),
        })
        .sort({ countryName: 1 })
        .exec();
      ...
      if (results.length === 0) {
        ...
        results = await this.countryModel
          .find({
            country_name: new RegExp(`^${this.escapeRegex(searchTerm)}$`, 'i'),
          })
          .sort({ country_name: 1 })
          .exec();
      }
